defmodule Uppy.CoreAbstractSchemaTest do
  ...
  describe "find_permanent_multipart_upload: " do
    test "returns record key in permanent path and e_tag not nil", context do
      ...
      assert {:ok, %{id: ^schema_data_id}} =
        Core.find_permanent_multipart_upload(
          {"user_avatar_objects", Schemas.UserAvatarObject},
          %{id: schema_data_id}
        )
    end
  ...
  describe "find_completed_multipart_upload: " do
  ...
  describe "find_temporary_multipart_upload: " do
  ...
  describe "find_parts: " do
    test "returns a list of parts from storage", context do
      user_id = context.user.id
      user_avatar_id = context.user_avatar.id
  ...
  describe "presigned_part: " do
    test "returns presigned part payload and schema data", context do
      user_id = context.user.id
      user_avatar_id = context.user_avatar.id
      ...
      assert {:ok, presigned_part_response} =
        Core.presigned_part(
          @bucket,
          {"user_avatar_objects", Schemas.UserAvatarObject},
          %{id: schema_data_id},
      ...
      assert "https://url.com/#{temp_key}" === presigned_url
  ...
  describe "complete_multipart_upload: " do
    test "updates e_tag of record and creates post processing job", context do
      ...
      assert {"user_avatar_objects", Schemas.UserAvatarObject} = ObanUtil.decode_binary_to_term(process_upload_job_query)
    end
  ...
  describe "abort_multipart_upload: " do
    test "deletes temporary record and schedules garbage collection", context do
      ...
      assert {:ok, abort_multipart_upload_response} =
      ...
      } = abort_multipart_upload_response
      ...
      assert {:error, %{code: :not_found}} = DBAction.find({"user_avatar_objects", Schemas.UserAvatarObject}, %{id: schema_data_id})
      ...
      assert {"user_avatar_objects", Schemas.UserAvatarObject} = ObanUtil.decode_binary_to_term(garbage_collect_object_job_query)
    end
  ...
  describe "start_multipart_upload: " do
    test "creates record, creates presigned upload, and creates job", context do
      ...
      assert {:ok, multipart_upload_response} =
      ...
      assert multipart_upload_key === temp_key
      ...
      assert %Oban.Job{
        args: %{
          ...
          bucket: "uppy-test",
          query: abort_multipart_upload_job_query,
          id: ^schema_data_id
        },
        attempt: 0,
        attempted_at: nil,
      ...
      assert {"user_avatar_objects", Schemas.UserAvatarObject} = ObanUtil.decode_binary_to_term(abort_multipart_upload_job_query)
    end
    ...
    test "creates record, creates presigned upload, and does not create job when scheduler disabled", context do
      ...
      assert {:ok, start_multipart_upload_response} =
        Core.start_multipart_upload(
          @bucket,
          user_id,
          {"user_avatar_objects", Schemas.UserAvatarObject},
      ...
      assert %{
        ...
        multipart_upload: multipart_upload,
        schema_data: schema_data
      } = start_multipart_upload_response
  ...
  describe "process_upload: " do
    test "processes object and updates record", context do
      ...
      assert %Uppy.Resolution{
        ...
        context: %{},
        private: %{},
        query: {"user_avatar_objects", Schemas.UserAvatarObject},
        resource: "user-avatars",
        state: :resolved,
  ...
  describe "start_upload: " do
    test "creates record, creates presigned upload, and creates job", context do
      ...
      assert {:ok, start_upload_response} =
        Core.start_upload(
          @bucket,
          user_id,
          {"user_avatar_objects", Schemas.UserAvatarObject},
      ...
      assert {"user_avatar_objects", Schemas.UserAvatarObject} = ObanUtil.decode_binary_to_term(abort_upload_job_query)
    end
