defmodule Uppy.Schedulers.ObanScheduler.WorkerAPI do
  ...
  ...
  """
  @spec query_to_arguments(query :: Ecto.Queryable.t() | {binary(), Ecto.Queryable.t()}) :: %{
          source: binary(),
          query: binary()
        }
  ...
  ...
  """
  @spec query_from_arguments(%{binary() => binary()}) ::
          Ecto.Queryable.t() | {binary(), Ecto.Queryable.t()}
  ...
  def move_upload(bucket, destination_object, query, id, pipeline, opts) do
    Uploader.move_upload(
      ...
      query,
      %{id: id},
      Keyword.put(opts, :pipeline, pipeline)
    )
  end
end
