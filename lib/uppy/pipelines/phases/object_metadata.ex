defmodule Uppy.Pipelines.Phases.ObjectMetadata do
  @moduledoc """
  ...
  """
  alias Uppy.{
  ...
  @logger_prefix "Uppy.Pipelines.Phases.ObjectMetadata"

  # ~256 bytes is needed to detect the file type
  @two_hundred_fifty_six_bytes 256

  def find_private(input) do
    Input.find_private(input, __MODULE__)
  end
  ...
  ### Examples
      ...
      iex> Uppy.Pipelines.Phases.ObjectMetadata.describe_object("bucket", "object")
  """
  def describe_object(bucket, object, options \\ []) do
    Utils.Logger.debug(
      @logger_prefix,
      "describe_object bucket=#{inspect(bucket)}, object=#{inspect(object)}"
    )
    ...
    with {:ok, data} <- download_head(bucket, object, options),
      ...
      case FileType.from_io(file) do
        {:error, :unrecognized} ->
          ...
          Utils.Logger.debug(
            @logger_prefix,
            "describe_object OK mimetype=#{inspect(mimetype)}, extension=#{inspect(extension)}"
          )
  ...
  def download_head(bucket, object, options \\ []) do
    ...
    Utils.Logger.debug(
      @logger_prefix,
      "download_head bucket=#{inspect(bucket)}, object=#{inspect(object)}, start_byte=#{inspect(start_byte)}, end_byte=#{inspect(end_byte)}"
    )
    ...
    with {:ok, {_start_byte, body}} <-
      ...
      Utils.Logger.debug(
        @logger_prefix,
        "download_head OK byte_size=#{:erlang.byte_size(body)} bytes"
      )
  ...
  defp end_byte!(options) do
    case options[:pipeline][:mimetype][:end_byte_size] do
      ...
      end_byte when end_byte <= @two_hundred_fifty_six_bytes ->
        end_byte

      term ->
        raise "Expected option `:end_byte` to be between 0..#{@two_hundred_fifty_six_bytes}, got: #{inspect(term)}"
    end
  end
end
